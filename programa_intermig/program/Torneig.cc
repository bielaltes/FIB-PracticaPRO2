...
void imprimir_torneig(BinTree<int>& a, bool left, bool pare,int prof,  vector<string>& particip) {
    if (not a.empty()) {
        BinTree<int> l = a.left();
        BinTree<int> r = a.right();
        if (not l.empty()) cout << "(";
        else {
            cout << a.value() << "." << particip[a.value()-1];
            if (left) cout << " ";
        }
        imprimir_torneig(l, true, left ,prof+1,particip);
        imprimir_torneig(r, false, not left,prof+1,particip);
        if (not left) {
            cout << ")";
...
void Torneig::iniciar_torneig(Cjt_jugadors& jugadors) {
    ...
    imprimir_torneig(a, true, true, 1, participants);
    cout << endl;
}
