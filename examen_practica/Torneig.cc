...
void Torneig::crear_emparellaments(BinTree<pair<int,string>>& a, int m) {
    ...
    //if ((m+1-((a.value()).first) <= num_participants)){
        if (((a.value()).first) %2 == 0) {

            if ( (m-((a.value()).first)) + 2 <= num_participants) {
                BinTree<pair<int,string>> l(pair<int,string>((a.value()).first, " "));
                BinTree<pair<int,string>> r(pair<int,string>((m-((a.value()).first)) + 2, " "));
            ...
            if ( (m-((a.value()).first)) <= num_participants) {
                if (m == 2) {
                    BinTree<pair<int,string>> l(pair<int,string>(1, " "));
                    BinTree<pair<int,string>> r(pair<int,string>(2, " "));
                    crear_emparellaments(l,2*m);
                    crear_emparellaments(r,2*m);
                    a = BinTree<pair<int,string>>(pair<int,string>(((a.value()).first), ""),l,r);
                }
                    ...
                    BinTree<pair<int,string>> l(pair<int,string>((a.value()).first, " "));
                    BinTree<pair<int,string>> r(pair<int,string>((m-((a.value()).first)), " "));
                    crear_emparellaments(l,2*m);
                    crear_emparellaments(r,2*m);
                    a = BinTree<pair<int,string>>(pair<int,string>(((a.value()).first), ""),l,r);
                }
...
void Torneig::consultar_torneig() {
    if (jugat and not jugantse) {
        imprimir_resultats(partits);
        cout << endl;
    }
    else if (jugantse) cout << "error: torneo en juego" << endl;
    else if (not jugat) cout << "error: torneo no disputado" << endl;
}
